...
class Svg_Sanitizer
{
    ...
    public static function sanitize(string $markup): string
    {
        ...
        foreach (iterator_to_array($xpath->query('//*')) as $element) {
            ...
            if (in_array($name, self::REJECT_ELEMENTS, true)) {
                throw new \InvalidArgumentException(esc_html(sprintf('SVG element <%s> is not allowed.', $element->localName)));
            }
    ...
    private static function vet_attribute(\DOMAttr $attr): void
    {
        ...
        if (str_starts_with($name, 'on')) {
            throw new \InvalidArgumentException(esc_html(sprintf('SVG event-handler attribute "%s" is not allowed.', $attr->name)));
        }
