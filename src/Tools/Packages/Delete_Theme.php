...
class Delete_Theme
{
    ...
    public function handle(array $args): array
    {
        ...
        if (! $theme->exists()) {
            throw new \RuntimeException(esc_html("Theme \"{$stylesheet}\" was not found."));
        }

        $active_stylesheet = get_stylesheet();
        $active_template   = get_template();
        if ($stylesheet === $active_stylesheet || $stylesheet === $active_template) {
            throw new \RuntimeException(esc_html("Refusing to delete active theme \"{$stylesheet}\"."));
        }
        ...
        if (is_wp_error($result)) {
            throw new \RuntimeException(esc_html('Theme delete failed: ' . $result->get_error_message()));
        }
