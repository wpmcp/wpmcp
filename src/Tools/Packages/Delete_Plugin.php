...
class Delete_Plugin
{
    ...
    public function handle(array $args): array
    {
        ...
        if (Package_Guard::is_protected_plugin($plugin)) {
            throw new \RuntimeException(esc_html("Refusing to delete protected plugin \"{$plugin}\"."));
        }
        ...
        if (! isset($all_plugins[ $plugin ])) {
            throw new \RuntimeException(esc_html("Plugin \"{$plugin}\" was not found."));
        }

        if (is_plugin_active($plugin)) {
            throw new \RuntimeException(esc_html("Refusing to delete active plugin \"{$plugin}\"; deactivate it first."));
        }
        ...
        if (is_wp_error($result)) {
            throw new \RuntimeException(esc_html('Plugin delete failed: ' . $result->get_error_message()));
        }
