...
class Install_Plugin
{
    ...
    public function handle(array $args): array
    {
        ...
        if (is_wp_error($info)) {
            throw new \RuntimeException(esc_html('Could not find plugin "' . $slug . '" on wordpress.org: ' . $info->get_error_message()));
        }
        ...
        if (is_wp_error($result) || ! $result) {
            ...
            throw new \RuntimeException(esc_html('Plugin install failed: ' . $message));
        }
