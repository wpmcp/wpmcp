...
4. **`delete-media` file recovery.** Force-deleting media (or deleting without `MEDIA_TRASH` enabled) permanently unlinks the physical file; rollback restores the media record (post, meta, terms) but not the file bytes. The response signals this via `files_recoverable: false` and a `warning` field. Full file-level recovery is tracked in issue #24. Media force-delete is disabled by default (`wpmcp_enable_delete_media` filter must be explicitly enabled).
