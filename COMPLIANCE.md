...
| B-20 | WPORG-17-TRADEMARK | ~~`readme.txt:3` — tag `claude` (dist: reviewer-discretion)~~ **Resolved (#169).** | Third-party trademark as a tag. Dropped from `readme.txt` and from `scripts/flavors/woocommerce/readme.txt`, which the engine never scans because `Plugin_Source::readme()` only resolves the root readme; `tests/free/Compliance/ShippedReadmesTest.php` now guards every shipped readme. A `wporg-free` run over the tree no longer reports a tag finding. |
