...
| B-19 | WPORG-17-TRADEMARK | ~~`readme.txt:1` — "WordPress" mid-title (dist: reviewer-discretion)~~ **Fixed (#168)** | Every shipped display name is now `WP MCP - MCP Server with Snapshot Undo for AI Agents` (root pair and `scripts/flavors/wporg/*`) or `WP MCP for WooCommerce` (`scripts/flavors/woocommerce/*`); none carries the restricted term. `tests/free/Compliance/PluginNameParityTest.php` re-checks all three pairs, and `scripts/build-woo-release.sh` gates the pair it stages. The remaining leading "WP" is the tolerated warning documented in WPORG-SUBMISSION.md. |
