...
| L-02 | PCP-ESCAPING | `src/Tools/WidgetBuilder/Dynamic_Widget.php:105` (dist: likely-reject) | **Resolved.** Plugin Check reported `OutputNotEscaped` here because it does not follow calls. The code is correct: `Widget_Renderer::render()` at `src/Tools/WidgetBuilder/Widget_Renderer.php:42` escapes every interpolated value by control type, and the surrounding template is capability-gated author markup. Fixed with a justified `phpcs:ignore` recording that, not more escaping. |
