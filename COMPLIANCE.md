...
| B-26 | PC-only | Fixed on `wip/issue-175` (#175). Was 4 × `WordPressVIPMinimum.Performance.WPQueryParams.SuppressFilters` at `src/Tools/WidgetBuilder/Widget_Registry.php`, `src/Tools/WidgetBuilder/Widget_Spec_Store.php`, `src/Tools/Elementor/List_Code_Snippets.php`, `src/Tools/BlockBuilder/Block_Spec_Store.php`. The paths quoted from the original scan (`src/Tools/Widgets/`, `src/Tools/Code/`, `src/Tools/Blocks/`) never existed; the two `src/Memory/Memory_Store.php` sites were missed by it. | Argument dropped at every catalog read, which is a behavioural no-op because `get_posts()` defaults the flag to true; the sniff fires on the explicit value. `Memory_Store::block_rules()` keeps it explicitly `true` under a justified `phpcs:ignore`, because `Memory_Guard::blocking_rule()` fails open and a `posts_*` filter that emptied that read would disable the write guard site-wide. Reintroduction is now caught by `PCP-SUPPRESS-FILTERS`. |
