...
| B-22 | PCP-FORBIDDEN-FUNCTIONS | 18 sites, all confirmed by Plugin Check (dist: resolved) | Closed out by #171. Converted: `unlink` ×7 → `wp_delete_file()`; `rmdir` → `WP_Filesystem::rmdir()` (`src/Safety/File_Backup.php:125`); `fopen`/`fread`/`fclose` in `Get_Debug_Log` and `readfile` in `src/Admin/Connection_Page.php:196` → `file_get_contents()`, which `plugin-check.ruleset.xml` excludes from the sniff (`file_get_contents_file_get_contents`); `parse_url` → `wp_parse_url()`; `wp_get_sidebars_widgets()` → `get_option('sidebars_widgets')` (`src/Tools/Structure/List_Sidebar_Widgets.php:27`). Annotated instead, because no WordPress equivalent exists: `fclose` ×3 on `proc_open()` pipe handles (`src/Tools/Cli/Wp_Cli_Executor.php:54`, `:88`, `:90`) and the single `curl_setopt(CURLOPT_RESOLVE)`, which moved out of `Page_Audit` to `src/Tools/Performance/Curl_Dns_Pin.php:36` and is deliberate SSRF defence. Both files are also removed outright from the wp.org zip by `scripts/flavors/wporg/strip.php`, so nothing in this row reaches Plugin Check; the annotations are what keeps the source tree green under this engine and under WordPressCS. |
