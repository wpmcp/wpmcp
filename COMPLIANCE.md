...
| R-07 | PC-only | Closed (#180). 8 × `WordPress.NamingConventions.PrefixAllGlobals`: 6 `NonPrefixedHooknameFound` (`src/Tools/I18n/I18n_Adapter.php:102`, `:161`, `:200`, `:209`, `src/Tools/Cache/Page_Cache_Detector.php:140`, `src/Tools/Security/Hardening_Audit.php:161`) plus 2 `DynamicHooknameFound` (`src/Tools/Cron/Run_Event.php:56`, `src/MCP/Request_Log.php:94`) | Every one of the eight is a cross-plugin or dynamic hook we invoke rather than define (WPML, LiteSpeed Cache, core's `xmlrpc_enabled`, an already-scheduled cron hook, and our own `wpmcp_`-prefixed capture filter). Renaming them would break the integration, so each carries a `phpcs:ignore` with its justification, which is the route the sniff supports. The flavor half of this finding is solved at bootstrap, not by rewriting identifiers: `src/flavor-guard.php` makes at most one build boot per request. A build-time `wpmcp_` prefix rewrite was tried and reverted (see the rationale in `scripts/build-woo-release.sh`). |
