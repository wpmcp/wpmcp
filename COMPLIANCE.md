...
| B-23 | PC-only | RESOLVED in 0.8.1: all four shipped headers now declare `Tested up to: 7.1`, the current release per api.wordpress.org stable-check | Was `Tested up to: 6.9`. Plugin Check errors: "your plugin will not show up in searches". Bumped after a full free-suite pass on WordPress 7.1, logged in `docs/release-checklist.md`. The engine still cannot see this offline (it is a live-version check by nature), so the recurrence gate is `tests/free/Release/ReleaseHeadersTest.php` plus gate 4b in `scripts/build-wporg-release.sh`. |
