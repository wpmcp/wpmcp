...
The header above, the counts under "Findings" and the reproduction commands under "Reproduce" describe the 0.8.0 scan that produced the baseline. Rows marked resolved or bumped in 0.8.1 record fixes that landed after that scan; the current version is the one `wpmcp.php` declares.
...
Everything else is tractable in a normal work week: the 18 `WordPress.WP.AlternativeFunctions` sites (now closed out, see B-22), one malformed ABSPATH guard, a readme title that trips the trademark check, and the escaping/DB warnings.
...
These are the audit-time counts for 0.8.0 and are the baseline the ledger below refers to. The `distribution` source-tree run has since become a CI gate (ENG-2, issue #186) and reports **0** blockers on `main`; its one remaining `likely-reject` is L-03.
...
| `WordPress.DB.PreparedSQL.NotPrepared` | 0 (was 19; pinned at 0 by `.phpcs-baseline.json`) | ERROR |
| `PluginCheck.Security.DirectDB.UnescapedDBParameter` | 0 / 0 (was 8 / 7; measured with Plugin Check 2.1.0's sniff after #174, 8 sites suppressed per site where the sniff cannot follow a variable format string into `prepare()`, see B-25) | ERROR / WARNING |
| `WordPress.Security.ValidatedSanitizedInput.*` | 11 | WARNING |
| `WordPress.DB.PreparedSQL.InterpolatedNotPrepared` | 0 reported, 2 suppressed (was 10): `Db_Dumper`'s multi-column `ORDER BY` list and `Rewrite_Site_Urls`' const-map column list | WARNING |
...
| B-07 | WPORG-05-TRIALWARE | `src/Tools/Media/Stock/Insert_Stock_Image.php:27` (dist: best-practice) | RESOLVED (issue #163). The ability leaves the build: the handler is in `REMOVED_PATHS` and the `'pro'` registration is swept by `remove_pro_abilities()`. Held by gate 3b of `scripts/build-wporg-release.sh` (no manifest pro-tier ability registered in the cut) and by `tests/free/Build/WporgStripTest.php`, which runs the real strip over a real staging. |
| B-08 | WPORG-06-LICENSING | `src/Freemius/Bootstrap.php` — `'anonymous_mode'` in `Bootstrap::config()` (dist: best-practice) | **Resolved (#164).** Guideline 7 requires "explicit and authorized consent... commonly done via an 'opt in' method". `anonymous_mode` suppressed the SDK's stock opt-in screen, so the SDK could reach `api.freemius.com` with no consent step the user ever saw. Closed on both of the routes the guideline allows: the directory build drops the SDK entirely (`scripts/flavors/wporg/strip.php` removes `src/Freemius`, `scripts/build-wporg-release.sh` composer-removes `freemius/wordpress-sdk`, and gate 3 fails the build if either survives), and the self-hosted zip now sets `anonymous_mode => false` with `enable_anonymous => true`, shipping the stock default-off connect screen with its Skip link. `menu.override_exact => true` keeps the SDK's activation-mode menu takeover on the activation URL instead of stripping the whole wpmcp admin menu, and the `plugin_icon` filter is pinned to `assets/wpmcp-icon.svg` (staged and gated by `scripts/build-release.sh`) so the connect screen makes no remote icon fetch before opt-in. |
| B-09 | WPORG-06-LICENSING | `composer.json:6`, `vendor/freemius`, `src/Freemius/Bootstrap.php:127`, `:167`, `:171`, `src/Freemius/wpmcp-fs.php:21`, `:26` (dist: best-practice) | Bundling the SDK is permitted and precedented (the Plugin Review Team's own ruleset carves out `*/freemius/*` at `plugin-review.xml:20`), but only once B-03 lands, because guideline 6 forbids "a service that exists for the sole purpose of validating licenses... while all functional aspects of the plugin are included locally". Today the SDK is exactly that. **Resolved for the directory build (#165):** the directory cut does not bundle the SDK at all. `scripts/flavors/wporg/strip.php` removes `src/Freemius` and `src/Pro` and rewrites every `Gate::is_pro()` / `Gate::can_use()` call site, the build then runs `composer remove freemius/wordpress-sdk`, and gate 3 fails the build if `Gate::`, `is_pro`, `freemius` or `fs_dynamic_init` survives into the shipped PHP. Unchanged for the distribution build. |
...
| B-18 | WPORG-11-ADMIN-NAG | `src/Tools/Compose/Build_Page.php:57` — "is a PRO feature" in a thrown message (dist: best-practice) | Guideline 9 prohibits "implying users must pay to unlock included features". Under `wporg-free` it stays a blocker and goes away with B-06. **Downgraded under `distribution` (issue #186):** `Profile::distribution()` sets the `pay_to_unlock_copy` option to best-practice, which `Admin_Nag_Rule` applies to its guideline 9 findings only, so the copy follows the `WPORG-05-TRIALWARE` gate it describes rather than outranking it. The admin-notice half of `WPORG-11` keeps its severity in both profiles. |
| B-19 | WPORG-17-TRADEMARK | ~~`readme.txt:1` — "WordPress" mid-title (dist: reviewer-discretion)~~ **Fixed (#168)** | Every shipped display name is now `WP MCP - MCP Server with Snapshot Undo for AI Agents` (root pair and `scripts/flavors/wporg/*`) or `WP MCP for WooCommerce` (`scripts/flavors/woocommerce/*`); none carries the restricted term. `tests/free/Release/ReleaseHeadersTest.php` re-checks all three pairs (parity, and `Trademark_Rule` over each name), and `scripts/build-woo-release.sh` runs the same rule over the pair it stages. The remaining leading "WP" is the finding the rule itself marks best-practice (no profile promotes it to blocker; `distribution` prints it at reviewer-discretion), the tolerated warning documented in WPORG-SUBMISSION.md. |
| B-20 | WPORG-17-TRADEMARK | RESOLVED in 0.8.1: `claude` is gone from every shipped tag list | Third-party trademark as a tag. The list is at the 5-tag maximum anyway, so removing it cost nothing: `readme.txt` took `undo` and the WooCommerce flavor took `automation`. `ReleaseHeadersTest` keeps it out. |
...
| B-22 | PCP-FORBIDDEN-FUNCTIONS | 18 sites, all confirmed by Plugin Check (dist: blocker) | **Resolved (#219; the conversions landed with #157, the four annotations and the engine support for them with #202, exact-code matching with #215).** Converted: `unlink` ×7 → `wp_delete_file()`; `rmdir` → `WP_Filesystem::rmdir()` (`src/Safety/File_Backup.php:125`); `fopen`/`fread`/`fclose` in `src/Tools/Diagnostics/Get_Debug_Log.php` and `readfile` in `src/Admin/Connection_Page.php` → `file_get_contents()`, which `plugin-check.ruleset.xml` excludes from the sniff (see the `Forbidden_Functions_Rule::ALTERNATIVES` docblock); `parse_url` → `wp_parse_url()`; `wp_get_sidebars_widgets()` → `get_option('sidebars_widgets')` (`src/Tools/Structure/List_Sidebar_Widgets.php:27`). Four sites carry a justified `phpcs:ignore` instead, because no WordPress equivalent exists: the three `fclose()` calls on `proc_open()` pipe handles in `src/Tools/Cli/Wp_Cli_Executor.php` (`:54`, `:88`, `:90`; `WP_Filesystem` cannot close a process pipe) and `curl_setopt(CURLOPT_RESOLVE)`, which moved out of `Page_Audit` to `src/Tools/Performance/Curl_Dns_Pin.php:36` and is the SSRF pin the HTTP API has no equivalent for. That justification vouches for the pin being scoped to one request, so #219 also wraps the `wp_safe_remote_get()` in `Page_Audit::fetch()` in `try`/`finally`: the `http_api_curl` filter is removed even when a third-party `pre_http_request` or `http_api_curl` hook throws, where it previously stayed pinned for every later WP curl request to that host:port in the process (`tests/free/Performance/PageAuditSsrfTest.php` pins that). Both annotated files are also removed from the wp.org zip by `scripts/flavors/wporg/strip.php`, so nothing in this row reaches Plugin Check; the annotations are what keeps the source tree green under this engine and under WordPressCS. See ENG-3 for how the engine honours them. The engine now reports nothing under this rule in either profile except the `set_time_limit()` at L-03. |
| B-23 | PC-only | Header bumped in 0.8.1, artifact verification pending: all four shipped headers now declare `Tested up to: 7.1`, the current release per api.wordpress.org stable-check | Was `Tested up to: 6.9`. Plugin Check errors: "your plugin will not show up in searches". Bumped after the free suite ran on WordPress 7.1 (2784 tests; the four `RestoreControllerAjaxTest` failures in that run are WooCommerce's first-install path, not core), logged in `docs/release-checklist.md`. The per-artifact manual smoke rows in that log are still `pending`, so issue #172 stays open until they are filled. The engine still cannot see this offline (it is a live-version check by nature), so the recurrence gate is `tests/free/Release/ReleaseHeadersTest.php` (which also pins the WordPress version CI installs to the same floor) plus gate 4b in `scripts/build-wporg-release.sh`. |
...
| B-25 | PC-only | **Fixed (issue #174).** Was 19 × `WordPress.DB.PreparedSQL.NotPrepared`, worst `src/Tools/Database/Database_Guard.php` (6) and `src/Safety/Snapshot_Store.php` (6), plus 8 × `PluginCheck.Security.DirectDB.UnescapedDBParameter` at ERROR | Every single-identifier interpolation is bound with `wpdb::prepare()`'s `%i` placeholder (WordPress 6.2+; the plugin requires 6.9) across `Database_Guard`, `Snapshot_Store`, `Describe_Table`, `List_Transients`, `List_Operations`, `Redirect_Store`, `Search_Index_Store`, `Db_Dumper` (including `SHOW CREATE TABLE`, `SHOW KEYS` and `SHOW COLUMNS`), `Paid_Memberships_Pro_Integration` and `Gravity_Tables_Integration`. Measured on the tree merged with main: `phpcs --standard=phpcs-wporg.xml.dist` reports `NotPrepared` 0 (the #185 baseline recorded 36), `InterpolatedNotPrepared` 0 (was 24) and `DirectDatabaseQuery.SchemaChange` 0 (was 1); Plugin Check 2.1.0's own `PluginCheck.Security.DirectDB` sniff over `src` reports 0 ERROR / 0 WARNING. Those zeros are partly comment-driven, and the row says so: eight `$wpdb` calls keep a scoped `phpcs:ignore` naming `WordPress.DB.PreparedSQL.NotPrepared` and `PluginCheck.Security.DirectDB.UnescapedDBParameter` because their format string is assembled in a variable from literal `%i`/`%s`/`%d` fragments and bound on the next line, which neither sniff can follow through the variable (`Database_Guard::before_image()`, `List_Operations::handle()` twice, `Redirect_Store::all()` and `count()`, `Search_Index_Store::candidates()`, `List_Transients::handle()`, `Db_Dumper::dump_table()`). Without those ignores the Plugin Check sniff reports each of the eight as a WARNING, never an ERROR, so the ERROR count is 0 on the code alone and the ignores are what take the WARNING count from 8 to 0. Two further ignores cover what `%i` cannot express: the caller-supplied read-only SQL that is `Query`'s purpose (gated by `Database_Guard::is_read_only_sql()`) and the multi-column `ORDER BY` list from `Db_Dumper::column_order()`. `.phpcs-baseline.json` now pins `NotPrepared`, `InterpolatedNotPrepared` and `SchemaChange` at 0 and `NotPrepared` is out of the Plugin Check job's `ignore-codes`, so the next interpolated identifier fails CI twice. |
| B-26 | PCP-SUPPRESS-FILTERS | **Resolved (#215).** Was 4 × `WordPressVIPMinimum.Performance.WPQueryParams.SuppressFilters` at `src/Tools/WidgetBuilder/Widget_Registry.php`, `src/Tools/WidgetBuilder/Widget_Spec_Store.php`, `src/Tools/Elementor/List_Code_Snippets.php`, `src/Tools/BlockBuilder/Block_Spec_Store.php`. The paths quoted from the original scan (`src/Tools/Widgets/`, `src/Tools/Code/`, `src/Tools/Blocks/`) never existed; the two `src/Memory/Memory_Store.php` sites were missed by it and were annotated per site by #185. | Argument dropped at every catalog read and at `Memory_Store::query()`, which is a behavioural no-op because `get_posts()` defaults the flag to true; the sniff fires on the explicit value. `Memory_Store::block_rules()` keeps it explicitly `true` under a justified `phpcs:ignore` for the full message code `WordPressVIPMinimum.Performance.WPQueryParams.SuppressFilters_suppress_filters` (PHPCS matches annotations on whole dot-separated segments, so the shorter `...WPQueryParams.SuppressFilters` suppresses nothing), because `Memory_Guard::blocking_rule()` fails open and a `posts_*` filter that emptied that read would disable the write guard site-wide. Reintroduction is a blocker under `PCP-SUPPRESS-FILTERS`, which honours the same annotation Plugin Check does. |
| B-27 | PC-only | **Resolved (#176; the source fix landed with #210).** 3 × `WordPress.WP.I18n.MissingTranslatorsComment` in `src/Admin/Audit_Log_Page.php` (now lines 120, 175 and 183), plus 1 × `WordPress.WP.I18n.NonSingularStringLiteralText` at `src/Admin/Memory_Page.php:133`, where `esc_html__()` took a three-part concatenation that make-pot cannot extract. The count of 1 and the line-50 reference in earlier revisions of this row were both wrong: there were three sites and none of them was line 50. | Each `__()` carries a `/* translators: */` comment on the line above it and the description is one literal. `WordPress.WP.I18n` runs in CI through `phpcs-wporg.xml.dist` and `composer lint` (#185), and `.phpcs-baseline.json` records no I18n codes, so the next occurrence of either sniff fails the build. The engine's `I18n_Rule` keeps to the text-domain half of that sniff and does not duplicate the two checks phpcs already runs. |
...
| L-02 | PCP-ESCAPING | `src/Tools/WidgetBuilder/Dynamic_Widget.php:105` (dist: likely-reject) | **Resolved.** Plugin Check reported `OutputNotEscaped` here because it does not follow calls. The code is correct: `Widget_Renderer::render()` in `src/Tools/WidgetBuilder/Widget_Renderer.php` escapes every interpolated value by control type, and the surrounding template is capability-gated author markup (`Widget_Spec_Store` runs it through `wp_kses_post` on write for any author without `unfiltered_html`). Fixed with a justified `phpcs:ignore` recording that, not more escaping. |
...
| L-04 | WPORG-12-README | ~~`readme.txt:1` — title differs from the `Plugin Name: wpmcp` header (dist: best-practice)~~ **Fixed (#168)** | The header carries the real display name and the readme title is byte-identical, in all three artifact pairs. `mismatched_plugin_name` had never applied to the directory zip, which is staged from `scripts/flavors/wporg/*`; it applied to the root pair that `scripts/build-release.sh` ships and to the WooCommerce pair, which was mismatched too and is now gated at build time. |

### Reviewer discretion

| ID | Rule | Evidence | Note |
|---|---|---|---|
| R-01 | WPORG-08-PLUGIN-INSTALL | 19 engine hits across `src/Tools/Packages/*` and the handler constructors in `src/Plugin.php` (dist: reviewer-discretion) | The reviewer checklist says "A plugin can be required but not included or auto-installed", and guideline 8 forbids "serving updates or otherwise installing plugins". These install from wp.org through core's own `Plugin_Upgrader` on an explicit request carrying the granular core capability the matching wp-admin screen requires (`install_plugins`, `activate_plugins`, `update_plugins`, `delete_plugins`, `switch_themes`, `install_themes`, `update_themes`, `delete_themes`; the installers' `activate: true` step checks the second capability too), which is the defensible reading, but it is the third most likely thing to sink the submission. **Answered** for 0.8.1: the prepared reviewer reply is `WPORG-SUBMISSION.md` section 5.5 with the long form, site table and the engine-count explanation in `docs/wporg/guideline-8-install-abilities.md` (issue #179). |
...
| R-06 | WPORG-08-UPDATER | `vendor/freemius/wordpress-sdk/includes/class-freemius.php` references `site_transient_update_plugins` | `Plugin_Updater_Check` is a static regex with **no** vendor carve-out (the `*/freemius/*` exclusion at `plugin-review.xml:20` applies only to PHPCS). Expect a Plugin Check error at submission. Our local run did not surface it because Plugin Check excludes `vendor/` by default in the CLI; the wp.org-side run does not necessarily. **Resolved for the directory build (#165):** `scripts/build-wporg-release.sh` runs `composer remove freemius/wordpress-sdk` in the stage, so the SDK and its updater are not vendored at all, and three checks assert it rather than trusting the strip. `scripts/lib/updater-gate.sh` greps the pre-zip tree, `vendor/` included, for `Updater_Rule::UPDATER_PATTERNS` (one shared list, read at run time); `Updater_Rule` widens the same patterns over `vendor/` for artifact scans, so the engine run against the extracted zip is not blind to it; and the `plugin-check` CI job runs the real Plugin Check on the extracted zip (that job cannot see `vendor/`, which Plugin Check excludes unconditionally, so the first two checks are the ones that cover it). The written explanation for the reviewer is `WPORG-SUBMISSION.md` 5.6. Still open for the distribution build, where the SDK legitimately ships. |
| R-07 | PC-only | 8 × `WordPress.NamingConventions.PrefixAllGlobals`: 6 `NonPrefixedHooknameFound` (`src/Tools/I18n/I18n_Adapter.php:102`, `:161`, `:200`, `:209`, `src/Tools/Cache/Page_Cache_Detector.php:140`, `src/Tools/Security/Hardening_Audit.php:161`) plus 2 `DynamicHooknameFound` (`src/Tools/Cron/Run_Event.php:56`, `src/MCP/Request_Log.php:94`) | **Resolved (issue #180).** Every one of the eight is a cross-plugin or dynamic hook we invoke rather than define (WPML, LiteSpeed Cache, core's `xmlrpc_enabled`, an already-scheduled cron hook, and our own `wpmcp_`-prefixed capture filter). Renaming them would break the integration, so each carries a `phpcs:ignore` with its justification, which is the route the sniff supports; both codes are gone from `.phpcs-baseline.json` (the ratchet treats an absent code as zero). The flavor half of this finding is solved at bootstrap, not by rewriting identifiers: `src/flavor-guard.php` ranks the active WP MCP builds by the `WPMCP Flavor:` header in each main file (full > wporg > woocommerce) and makes every lower-ranked build stand down with an admin notice, so at most one boots per request whichever directory each lives in. A build-time `wpmcp_` prefix rewrite was tried and reverted (see the rationale in `scripts/build-woo-release.sh`). The vertical build's text-domain rewrite initially also rewrote the guard's `'wpmcp'` basename prefix, which inverted the precedence in the shipped zip (the full plugin stood down); the guard is now excluded from that rewrite, the build fails unless the staged guard is byte-identical to the source tree, and CI runs `scripts/build-woo-release.sh` so the artifact is gated, not just the tree. |
| R-08 | PC-only | 3 × `error_log_set_error_handler` in `src/Tools/Export/Export_Content.php:164`, `:243`, `:282`; 1 × `WordPress.PHP.NoSilencedErrors` at `:309` | `WordPress.PHP.DevelopmentFunctions`. **Resolved (issue #181).** Every call carries a scoped `phpcs:ignore` with an in-place justification. The handler installed at `:164` is narrow by construction: it claims only the `Cannot modify header information` warning `export_wp()`'s own `header()` calls raise and chains everything else to the handler that was installed before it (returning `false` only when there was none), so warnings from third-party callbacks on the export hooks still reach the site's own error handling or PHP's. It is removed by a bounded probe-and-pop unwind in the `finally` block that stops at the frame beneath the one this class pushed, so it neither leaves the suppressor live under a leftover handler nor strips handlers that were installed before the tool ran; `:243` and `:282` are that unwind's read-only probes (PHP exposes no getter). The `@ob_end_clean()` at `:309` is the buffer drain honouring a non-removable buffer through its return value rather than a notice. Regression tests in `tests/free/Export/ExportContentTest.php` pin restoration after a throw, after a hook leaves a handler or a null frame installed, after a hook pops one frame too many, the handler's message discrimination and chaining, and capture when a hook closes or nests a buffer. |
...
| ENG-2 | `.github/workflows/ci.yml`, `compliance` job | **Resolved (issue #186).** Neither check step carries `continue-on-error`; the job is a gate. The distribution-profile step runs the engine on the source tree and fails on any blocker, and the wporg-free step fails through `scripts/build-wporg-release.sh`, which runs the engine on the built zip as its last step. Removing the flag exposed five source-tree blockers under the distribution profile, cleared in the same change: three `fclose()` sites and one `curl_setopt()` site by justified annotation (B-22), and the `Build_Page.php` pay-to-unlock copy by the profile option (B-18). The gate was verified red on the first run with the flag gone (https://github.com/wpmcp/wpmcp/actions/runs/33296218275) and green once those sites were cleared. |
| ENG-3 | `tools/compliance/src/Source_File.php`, `has_phpcs_ignore()` | The engine honours a justified `phpcs:ignore` in both profiles, the way PHPCS and Plugin Check do, in `Forbidden_Functions_Rule`, `Input_Sanitization_Rule` and `Output_Escaping_Rule`. The annotation must carry a justification after `--` (a bare one suppresses nothing), must sit on the call line or stand alone on the line above (a trailing annotation on the line above belongs to that line), and must name the exact message code PHPCS would emit for that call, or a whole-segment ancestor of it: an ignore written for `file_system_operations_fopen` does not cover the `fclose()` beside it, and `WordPress.WP.Alt` covers nothing. Calls a second WordPressCS sniff also reports (`ini_set()` under `WordPress.PHP.IniSet`, `dl()` and `str_rot13()` under `WordPress.PHP.DiscouragedPHPFunctions`) accept either code. `phpcs.xml.dist` is still PSR-12 only (ENG-1), so the engine is the only thing in CI that reads these codes. |
...
- The Freemius SDK in this configuration unlocks only locally-present code, which is guideline 6's named prohibition. The separate consent problem it used to carry (`anonymous_mode => true`, bypassing guideline 7's opt-in) is closed: see B-08. Guideline 6 is answered the same way as the rest of this section, by stripping the SDK from the directory build rather than by configuring it.
...
Two things to know about the CLI run. It excludes `vendor/` by default, so the bundled Freemius SDK is not scanned and R-06 will not appear locally even though it may at submission (there is no way to put `vendor/` back in scope: the CLI and the GitHub action both exclude it unconditionally, so `scripts/lib/updater-gate.sh` and the engine's artifact scan are the vendor-inclusive checks). And the CSV output repeats a header per file, so parse it by skipping `FILE:` lines and repeated header rows rather than handing it straight to a CSV reader.

### CI

`.github/workflows/ci.yml` has a `compliance` job that runs the distribution profile on the source tree, then builds the zip and runs `wporg-free --artifact` on it, and uploads a markdown report on every run. Both check steps are hard gates (ENG-2, issue #186): the engine exits 1 on any blocker, so either step failing blocks the merge. A change that reintroduces a blocker under either profile has to clear it, or carry a justified `phpcs:ignore` where the rule mirrors a sniff (ENG-3), before it can land.
