...
| R-08 | PC-only | 2 × `error_log_set_error_handler` — `src/Tools/Export/Export_Content.php:149`, `:202` | `WordPress.PHP.DevelopmentFunctions`. **Resolved (issue #181).** Both calls carry a scoped `phpcs:ignore` with an in-place justification. The handler installed at `:149` is narrow by construction: it claims only the `Cannot modify header information` warning `export_wp()`'s own `header()` calls raise and returns `false` for everything else, so warnings from third-party callbacks on the export hooks still reach PHP's normal handling. It is removed by a bounded probe-and-pop unwind in the `finally` block, which pops the handler this class pushed rather than whatever happens to be on top of the stack; `:202` is that read-only probe (PHP exposes no getter). Regression tests in `tests/free/Export/ExportContentTest.php` pin restoration after a throw, after a hook leaves a handler installed, and the handler's message discrimination. |
